/*
 * Ported from jellyfin-android (https://github.com/jellyfin/jellyfin-android),
 * file: app/src/main/java/org/jellyfin/mobile/player/deviceprofile/DeviceCodec.kt
 *
...
import dev.jdtech.jellyfin.player.CodecHelpers.getAudioCodec
import dev.jdtech.jellyfin.player.CodecHelpers.getAudioProfile
import dev.jdtech.jellyfin.player.CodecHelpers.getVideoCodec
import dev.jdtech.jellyfin.player.CodecHelpers.getVideoLevel
import dev.jdtech.jellyfin.player.CodecHelpers.getVideoProfile
import kotlin.math.max

internal sealed class DeviceCodec(
    val name: String,
    val mimeType: String,
            ...
            return when {
                videoCodec != null -> {
                    ...
                    Video(
                        ...
                        profiles = profiles,
                        levels = levels,
                        maxBitrate = codecCapabilities.videoCapabilities?.bitrateRange?.upper ?: 0,
                    )
                }
                audioCodec != null -> {
                    val profiles = HashSet<String>()
